...
//   events early -> match intent (metadata -> object_id) -> fn_confirm_monime_payment
//   (amount/currency check + guarded flip, done inside that SECURITY DEFINER
//   function so it's one atomic transaction).
//
// Payment Codes (USSD), not hosted checkout sessions: the completion event
// is `payment_code.completed`, object.type is "payment_code" (id pmc-...),
// and there's no ownershipGraph chain to walk for these events — metadata
// and direct object-id matching are the only two strategies that apply.
// A stale/abandoned code is left informational: the pg_cron sweep
// (fn_expire_monime_intents) reclaims the stock hold on its own timer.
...
function matchIntentLocators(evt: any): { metadataIntentId?: string; objectId?: string } {
  ...
  const metadataIntentId = (typeof dataMeta.intent_id === "string" && dataMeta.intent_id) || undefined;
  const objectId = evt?.object?.type === "payment_code" ? evt.object.id : undefined;
  return { metadataIntentId, objectId };
}
...
// Every event Monime fires that means "the money moved". payment_code.completed
// is what the USSD rail fires, but the payment.* pair is what carries
// `data.channel` (the actual rail: {type:"momo", provider:"m17", phoneNumber}).
// Recognising only payment_code.completed meant those were early-exited as
// non-completions and never linked to the intent — so the admin could never
// resolve a channel and showed a bare "Monime" for every order. It also meant a
// rail that fired only payment.completed would never confirm the order at all.
...
Deno.serve(async (req) => {
  ...
  const { metadataIntentId, objectId } = matchIntentLocators(evt);
  ...
  // The customer's money moved, but the intent had already been expired by the
  // reservation sweep or cancelled by staff. fn_confirm_monime_payment has filed
  // it into the refund queue (admin_payment_attention) for a human to resolve.
  // Ack it: a 500 would only make Monime redeliver an event nothing here can
  // settle, and the event.id dedup would swallow every retry anyway.
